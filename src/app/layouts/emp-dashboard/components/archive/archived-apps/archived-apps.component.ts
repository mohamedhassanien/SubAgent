...
@Component({
  selector: 'app-archived-apps',
  templateUrl: './archived-apps.component.html',
  styleUrls: ['./archived-apps.component.scss'],
})
export class ArchivedAppsComponent implements OnInit {
  email!: string;
  empId!: any;
  ...
  getArchivedApps() {
    ...
    this.employee.getArchivedApps(empId).subscribe(
      (data: any) => {
        if (data.length > 0) {
          console.log(data);
          return (this.myApplications = data);
  ...
  // getArchivedStudents() {
  //   this.employee.getArchivedStudents(this.empId).subscribe(
  //     (data: any) => {
  //       if (data.length > 0) {
  //         console.log(data);
  ...
  restoreApps() {
    ...
    this.employee
      .restoreArchivedApps(this.arrayOfEmail, empId)
      .subscribe((data) => {
        console.log(data);
      });
    this.getArchivedApps();
  ...
  // To restore archived students
  // restoreStudent(email: string) {
  //   let empId = String(localStorage.getItem('EmpName'));
  //   this.employee.restoreArchivedStudents(emails, empId).subscribe((data) => {
  //     console.log(data);
  //     this.getArchivedStudents();
  //   });
  // }
