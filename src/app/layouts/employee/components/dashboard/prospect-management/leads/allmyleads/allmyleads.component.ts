...
@Component({
  selector: 'app-allmyleads',
  templateUrl: './allmyleads.component.html',
  styleUrls: ['./allmyleads.component.scss'],
})
export class AllmyleadsComponent implements OnInit {
  ...
  extractSource(source: string) {
    const sources = source.replace(' ', '');
    if (sources.includes('/')) {
      let sourceArray: string[] = sources.split(',');
      return sourceArray;
    } else {
      return [sources];
    }
  ...
  // To get all students
  getStudents() {
    this._EmployeeService.getEmployeeStudentstotal().subscribe((data: any) => {
